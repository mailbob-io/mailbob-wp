...
use Mailbob\Common\Traits\Singleton;
...
final class Plugin {
	...
	/**
	 * Get the plugin metadata from the root file and include own data
	 *
	 * @return array
	 * @since 1.0.0
	 */
	public function data(): array {
		$plugin_data = apply_filters(
			...
			array(
				...
				'plugin_settings_page_url' => 'admin.php?page=mailbob',
				'plugin_path'              => untrailingslashit(
					plugin_dir_path( MAILBOB_PLUGIN_FILE )
				),
				'plugin_template_folder'   => 'templates',
				'ext_template_folder'      => 'mailbob-templates',
				'plugin_basename'          => plugin_basename( MAILBOB_PLUGIN_FILE ),
			)
		);
		return array_merge(
			apply_filters(
				'mailbob_plugin_meta_data',
				get_file_data(
					MAILBOB_PLUGIN_FILE,
					array(
						'name'         => 'Plugin Name',
	...
	/**
	 * Get the plugin folder basename
	 *
	 * @return string
	 * @since 1.0.0
	 */
	...
	/**
	 * Retrieves the URL of the plugin settings page.
	 *
	 * This method fetches the URL for the plugin's settings page from the plugin's data.
	 * It calls the `data()` method to get an array of plugin data and returns the
	 * value associated with the 'plugin_settings_page_url' key.
	 *
	 * @return string The URL of the plugin settings page.
	 */
	public function pluginSettingPageUrl() {
		return $this->data()['plugin_settings_page_url'];
	}
